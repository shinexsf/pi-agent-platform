...
function respond(id, ok, resultOrError) {
    const wire = ok
        ...
        : {
            ...
            ok: false,
            error: {
                message: resultOrError instanceof Error
...
async function dispatch(req) {
    ...
    try {
        switch (method) {
            case 'createSession': {
                const [config, sessionId, existingSessionPath] = args;
            ...
            case 'prompt': {
                ...
                const [message, promptOptions] = args;
                ...
                for (const att of promptOptions.attachments) {
                    ...
                    try {
                        ...
                        console.warn(`[worker] skip attachment ${att.id}: ${err.message}`);
                        const ext = extForMimeType(att.mimeType);
                        augmentedText += `\n<file name="${att.sha}.${ext}">[attachment load failed: ${err.message}]</file>`;
                        continue;
                    }
                    let processed;
                    try {
                        ...
                        console.warn(`[worker] resizeImage failed for ${att.id}: ${err.message}`);
                        const ext = extForMimeType(att.mimeType);
...
function readAttachmentBytes(sessionId, sha, mimeType) {
    if (!/^[a-f0-9]{64}$/.test(sha))
        throw new Error('sha format invalid');
    ...
    if (actual !== sha)
        throw new Error(`sha_mismatch: declared=${sha} actual=${actual}`);
