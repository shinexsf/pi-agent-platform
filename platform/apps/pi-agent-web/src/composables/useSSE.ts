...
export function useSSE(options: UseSSEOptions) {
  ...
  // ── Pending user message dedup (multi-device support) ──
  // When user sends from this client, we optimistically append a user bubble
  // with a temp ID. When the server echoes the same user message back via SSE,


  // ── File attachments for user messages (non-image files) ──
  // Keyed by messageId. Populated at message_end for user messages with <file attId> tags.
  ...
  /** Parse <file> tag attributes from a tag string. */
  ...
  /** Fetch attachments for a user message.
   *  Images → populates msg.images for ImageGrid.
   *  Non-image files → populates fileAttachmentsByMsg for file card rendering. */
  async function fetchUserMessageAttachments(msg: MessageDTO) {
    ...
    for (const m of allMatches) {
      ...
      if (mimeType.startsWith('image/')) {
        // Image → fetch and add to msg.images
        try {
        ...
        // Non-image file → track for file card rendering
  ...
  async function loadHistory() {
    try {
      ...
      // Fetch file attachments for user messages with <file attId> tags
      for (const msg of messages.value) {
  ...
  async function send(
    ...
    // Ensure SSE is connected — EventSource may have been closed by browser
    // after long idle (IM session timeout kills worker → no events → browser closes).
    ...
    // This avoids fragile content-matching dedup logic.
  ...
  function subscribeToSteerQueue() {
    // Web端通过SSE的queue_update事件接收steer队列，不需要bridge
    // queue_update事件已在subscribe()中处理
  }
